...
import { useGetAllCountSermonQuery } from '../../redux/features/sermon/sermonApi';
import { MdMissedVideoCall } from 'react-icons/md';
import { useTotalIncomeQuery } from '../../redux/features/storeOrder/storeOrderApi';
...
export default function DashboardStats({ }: Props) {
    ...
    const { data: s } = useGetAllCountSermonQuery<any>({});
                ...
                {isLoading ?
                    ...
                    <div className=' p-2 gap-4'>
                        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 mt-4 gap-5'>
                            ...
                            <div className='flex-1 bg-gray-900 rounded-lg px-3 py-6 relative border border-blue-300'>
                                <h1 className="text-[18px] md:text-[25px] text-white uppercase">Seed </h1>
                            ...
                            <div className='flex-1 bg-gray-900 relative rounded-lg px-3 py-6 border border-blue-300'>
                                ...
                                <h1 className="text-[18px] md:text-[25px] text-yellow-300">{formatCurrency(Store_income?.totalAmount)}</h1>
                                <div className='absolute top-3 right-3 bg-gray-700 rounded-md p-2 cursor-pointer'>
                            ...
                            <div className='flex-1 bg-gray-900 rounded-lg px-3 py-8 relative border border-blue-300'>
                                <h1 className="text-[18px] md:text-[25px] text-white uppercase">Sermon</h1>
                                <h4 className='text-blue-50 text-[16px] my-2'>Total sermons</h4>
                                <h1 className="text-[18px] md:text-[25px] text-yellow-300">{s?.sermonCount}</h1>
                                <div className='absolute top-3 right-3 bg-gray-700 rounded-md p-2 cursor-pointer'>
                                    <Link to="/sermon">
                                        <div>
                                            <MdMissedVideoCall className='text-2xl text-white font-semibold' />
                                        </div>
                                    </Link>
