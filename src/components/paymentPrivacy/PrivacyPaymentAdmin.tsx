...
import { useEditPaymentPolicyMutation, useGetPaymentAllPrivacyQuery } from '../../redux/features/paymentPrivacy/paymentPrivacyApi';



type Props = {}

export default function PrivacyPaymentAdmin({ }: Props) {
    const [open, setOpen] = useState(false)
    const toggleSidebar = () => setOpen(prev => !prev)

    const { data: dataP, refetch } = useGetPaymentAllPrivacyQuery({}, { refetchOnMountOrArgChange: true })
    const { data: Admin } = useAdminGetUsersQuery({})
    const [editPaymentPolicy, { isSuccess, isLoading, error }] = useEditPaymentPolicyMutation({})
    ...
    const handleSubmit = async () => {
        ...
        await editPaymentPolicy({ id, data })
