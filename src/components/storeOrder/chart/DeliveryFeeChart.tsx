...
import { useGetStoreDeliveryFeeAnalyticsQuery } from '../../../redux/features/analytics/analyticsApi';
...
export default function DeliveryFeeChart({ }: Props) {
    const { data } = useGetStoreDeliveryFeeAnalyticsQuery<any>({})
    const analyticsData = data?.order?.last12Months?.map((item: any) => ({
        name: item.month,
        Delivery_Payout: item.totalAmount
    }));
            ...
            <div className='border-b border-gray-700 shadow-md bg-gray-900'>
                <div className='max-container items-center padding-container pb-3 justify-between lg:justify-between flex flex-col lg:flex-row bg-gray-900'>
                    <div className='w-full px-4 md:px-0 md:w-[80%] m-auto mt-6 flex flex-row gap-5 items-center'>
                        ...
                        <div className='text-[25px] font-semibold text-white'>Deliver Fee Analysis</div>
                    </div>
                </div>
            </div>
                ...
                <div className="w-full">
                    <ResponsiveContainer width="100%" height={400}>
                        <AreaChart data={analyticsData} margin={{ top: 20, right: 30, bottom: 0 }}>
                            ...
                            <YAxis />
                            <Tooltip />
                            <Area type='monotone' dataKey="Delivery_Payout" stroke='#4d6249' fill='#4d62d9' />
                        </AreaChart>
                    </ResponsiveContainer>
                </div>
            </div>
