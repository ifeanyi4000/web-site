...
export default function OfferingLayout({ }: Props) {
    ...
    const search = (term: any) => {
        const filteredOfferings = data?.offerings.filter((user: any) => {
    ...
    const orderList = searchTerm ? searchResults : data?.offerings
    return (
        <div className=' h-screen overflow-x-hidden overflow-y-auto'>
            ...
            <div className='p-3'>
                ...
                <div className='flex flex-col md:flex-row gap-3 max-w-[80%'>
                    {isLoading ? <Loader /> : (
                        <OfferingsTable orderlist={orderList} />
                    )}
                </div>
